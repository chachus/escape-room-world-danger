...
hint_sound = sa.WaveObject.from_wave_file("hint_sound.wav")
    ...
    def processIncoming(self):
        ...
        while self.queue.qsize():
            try:
                ...
                instruction = msg[0:15]
                instruction.strip()
                print(instruction)
                if instruction == "start_timer":
                    self.start_countdown()
                if instructionmsg == "reset_timer":
                    self.reset_countdown()
                if instruction == "help":
                    while 1:
                        if not self.queue.empty():
                            break
                    hint_msg = self.queue.get(0)                    
                    self.show_hint(hint_msg)
                if instruction == "destroy_help":
                    self.destroy_hint()
                if instruction == "video1":
                    self.show_video("MisterX1.mp4")
                if instructiong == "video2":
                    self.show_video("MisterX2.mp4")
